 ...
 *
 * Adds a job application for a user (or an admin for them).
 *
...
/** GET /[username] => { user }
 *
 * Returns { username, jobs: [jobId1, jobId2, ...], firstName, lastName, isAdmin }
 *
 * Authorization required: login
 * getUserInfo(username): Retrieves user information including a list of applied job IDs.
 **/

router.get("/:username", ensureLoggedIn, ensureAdmin, ensureAdminOrSameUser, async function (req, res, next) {
  try {
    const userInfo = await User.getUserInfo(req.params.username);
